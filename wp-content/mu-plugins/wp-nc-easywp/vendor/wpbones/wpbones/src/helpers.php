...
if (!function_exists('wpbones_logger')) {
  ...
  function wpbones_logger()
  {
    if (count(func_get_args()) > 0) {
      return call_user_func_array([WPNCEasyWP()->log(), 'debug'], func_get_args());
    }

    return WPNCEasyWP()->log();
  }
}
...
if (!function_exists('wpbones_provider')) {
  ...
  {
    return WPNCEasyWP()->provider($provider);
  }
}
...
if (!function_exists('wpbones_modules')) {
  ...
  function wpbones_modules($file)
  {
    ...
    $path = WPNCEasyWP()->basePath . '/plugin/modules/' . $file_name;
